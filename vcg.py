...
class VCG:
    ...
    def compute(slot_clicks, reserve, bids):
        ...
        def total_payment(k):
            ...
            norm = max(slot_clicks)
            # pos = map(lambda s: float(s)/norm, c)
            ...
            if k >= n:
                ...
                def no_valids((a,bid)):
                    if a in allocation:
                        ...
                        return bid
                top_non_allocated = max(map(no_valids, bids))
                return pos[k] * max(reserve, top_non_allocated)
            else:
                return (pos[k] - pos[k+1]) * just_bids[k+1] + total_payment(k+1)
